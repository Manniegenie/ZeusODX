...
export default function TonDepositScreen() {
  ...
  useEffect(() => {
    const fetchTONAddress = async (): Promise<void> => {
      const cachedAddress = getCachedAddress('TON', 'TON');
      if (cachedAddress) {
        const addressData = cachedAddress.data || cachedAddress;
        setDepositData(addressData);
      } else {
        await handleGetTONAddress();
      }
    };
    fetchTONAddress();
  }, [getCachedAddress]);
  ...
  const handleGetTONAddress = async (): Promise<void> => {
    try {
      ...
      if (result.success) {
        ...
        showErrorMessage(result.error || 'Failed to get TON deposit address');
      }
    } catch (error) {
  ...
  const copyToClipboard = async (): Promise<void> => {
    if (!depositData?.address) {
      showErrorMessage('TON wallet address is not yet available');
      return;
    }
    try {
  ...
  const isLoading = isAddressLoading('TON', 'TON') || supportedLoading;
  const addressError = getAddressError('TON', 'TON');
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        >
          <View style={styles.headerSection}>
            <View style={styles.headerContainer}>
              ...
              <View style={styles.headerGroup}>
                <Text style={styles.headerTitle}>Deposit TON</Text>
                <Text style={styles.headerSubtitle}>Toncoin</Text>
              </View>

              <View style={styles.headerRight}>
                <HintBulb
                  title="Deposit tip"
                  hint="Only send Toncoin (TON) to this address on the TON network. Sending any other asset or using a different network may result in permanent loss."
                />
              </View>
            </View>
          </View>
          ...
          <View style={styles.subtitleSection}>
            <Text style={styles.subtitle}>Scan the QR code to get your TON deposit address</Text>
          </View>
            ...
            <View style={styles.qrContainer}>
              {isLoading ? (
                <View style={styles.loadingContainer}>
                  ...
                  <Text style={styles.loadingText}>Loading TON address...</Text>
                </View>
              ) : qrCodeData ? (
          ...
          <View style={styles.shareSection}>
            <TouchableOpacity
              style={[styles.shareButton, !depositData?.address && styles.shareButtonDisabled]}
              activeOpacity={0.8}
              disabled={!depositData?.address}
              onPress={async () => {
                if (!depositData?.address) {
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: Colors.text.primary, fontFamily: Typography.medium, fontSize: 18, textAlign: 'center', fontWeight: '600' },
  headerSubtitle: { color: Colors.text.secondary, fontFamily: Typography.regular, fontSize: 12, textAlign: 'center', marginTop: 2 },
  ...
  noQrText: { color: Colors.text.primary, fontFamily: Typography.medium, fontSize: 16, textAlign: 'center', marginBottom: 8 },
  placeholderText: { color: Colors.text.secondary, fontFamily: Typography.regular, fontSize: 14, textAlign: 'center' },
  ...
  addressContainer: {
    ...
    borderRadius: 12, padding: 14, borderWidth: 1, borderColor: '#E5E7EB', minHeight: 50,
  },
  ...
  copyButton: {
    ...
    elevation: 1, borderWidth: 1, borderColor: '#E5E7EB', overflow: 'hidden',
  },
  ...
  shareButtonDisabled: { backgroundColor: '#A5A6F6' },
