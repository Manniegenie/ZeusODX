...
export default function BnbEthDepositScreen() {
  ...
  useEffect(() => {
    const fetchBNBAddress = async (): Promise<void> => {
      const cachedAddress = getCachedAddress('BNB', 'ETH');
      if (cachedAddress) {
  ...
  const handleGetBNBAddress = async (): Promise<void> => {
    try {
      const result = await getBNBAddress('ETH');
      if (result.success) {
  ...
  const copyToClipboard = async (): Promise<void> => {
    ...
    try {
      ...
      Alert.alert('Copied!', 'BNB (Ethereum) wallet address copied to clipboard');
    } catch (error) {
      showErrorMessage('Failed to copy address to clipboard');
    }
  };

  const isLoading = isAddressLoading('BNB', 'ETH') || supportedLoading;
  const addressError = getAddressError('BNB', 'ETH');
