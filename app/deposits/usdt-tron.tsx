...
export default function UsdtTrxDepositScreen() {
  ...
  const handleGetTrxAddress = async (): Promise<void> => {
    try {
      const result = await getDepositAddress('USDT', 'TRX');
      if (result.success) {
        setDepositData(result.data);
        setShowError(false);
      } else {
        showErrorMessage(result.error || 'Failed to get Tron USDT deposit address');
      }
    } catch (error) {
  ...
  const copyToClipboard = async (): Promise<void> => {
    if (!depositData?.address) {
      showErrorMessage('Tron USDT wallet address is not yet available');
      return;
    }
    try {
        ...
        >
          <View style={styles.headerSection}>
            ...
            <View style={styles.headerGroup}>
              <Text style={styles.headerTitle}>Deposit USDT (TRX)</Text>
            </View>
          ...
          <View style={styles.warningSection}>
            ...
            <Text style={styles.warningText}>
              • Only send USDT on Tron (TRC20) network to this address{'\n'}
