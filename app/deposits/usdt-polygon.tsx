...
export default function UsdtPolygonDepositScreen() {
  ...
  useEffect(() => {
    const fetchPolygonAddress = async (): Promise<void> => {
      const cachedAddress = getCachedAddress('USDT', 'POLYGON');
      if (cachedAddress) {
        const addressData = cachedAddress.data || cachedAddress;
        setDepositData(addressData);
      } else {
        await handleGetPolygonAddress();
      }
    };
    fetchPolygonAddress();
  }, [getCachedAddress]);
  ...
  const handleGetPolygonAddress = async (): Promise<void> => {
    try {
      ...
      if (result.success) {
        ...
        showErrorMessage(result.error || 'Failed to get Polygon USDT deposit address');
      }
    } catch (error) {
  ...
  const copyToClipboard = async (): Promise<void> => {
    if (!depositData?.address) {
      showErrorMessage('Polygon USDT wallet address is not yet available');
      return;
    }
    try {
  ...
  const isLoading = isAddressLoading('USDT', 'POLYGON') || supportedLoading;
  const addressError = getAddressError('USDT', 'POLYGON');
        ...
        >
          <View style={styles.headerSection}>
            ...
            <View style={styles.headerGroup}>
              <Text style={styles.headerTitle}>Deposit USDT (Polygon)</Text>
            </View>
            <View style={styles.headerSpacer} />
          </View>

          <View style={styles.subtitleSection}>
            <Text style={styles.subtitle}>Scan the QR code to get Polygon deposit address</Text>
          </View>
            ...
            <View style={styles.qrContainer}>
              {isLoading ? (
                <View style={styles.loadingContainer}>
                  ...
                  <Text style={styles.loadingText}>Loading Polygon address...</Text>
                </View>
              ) : qrCodeData ? (
          ...
          <View style={styles.warningSection}>
            ...
            <Text style={styles.warningText}>
              • Only send USDT on Polygon network to this address{'\n'}
              • Sending from other networks may result in loss of funds{'\n'}
              • Ensure your wallet supports Polygon before sending
            </Text>
          </View>
