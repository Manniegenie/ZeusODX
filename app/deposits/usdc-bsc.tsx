...
export default function UsdcBscDepositScreen() {
  ...
  useEffect(() => {
    const fetchUSDCAddress = async (): Promise<void> => {
      const cachedAddress = getCachedAddress('USDC', 'BSC');
      if (cachedAddress) {
        const addressData = cachedAddress.data || cachedAddress;
        setDepositData(addressData);
      } else {
        await handleGetUSDCAddress();
      }
    };
    fetchUSDCAddress();
  }, [getCachedAddress]);
  ...
  const handleGetUSDCAddress = async (): Promise<void> => {
    try {
      const result = await getUSDCAddress('BSC');
      if (result.success) {
        setDepositData(result.data);
        setShowError(false);
      } else {
        showErrorMessage(result.error || 'Failed to get USDC deposit address');
      }
    } catch (error) {
  ...
  const copyToClipboard = async (): Promise<void> => {
    ...
    try {
      ...
      Alert.alert('Copied!', 'USDC (BSC) wallet address copied to clipboard');
    } catch (error) {
      showErrorMessage('Failed to copy address to clipboard');
    }
  };

  const isLoading = isAddressLoading('USDC', 'BSC') || supportedLoading;
  const addressError = getAddressError('USDC', 'BSC');
            ...
            <View style={styles.qrContainer}>
              {isLoading ? (
                <View style={styles.loadingContainer}>
                  ...
                  <Text style={styles.loadingText}>Loading USDC address...</Text>
                </View>
              ) : qrCodeData ? (
          ...
          <View style={styles.warningSection}>
            <View style={styles.warningContainer}>
              ...
              <Text style={styles.warningText}>
                Only send USDC on the Binance Smart Chain (BSC) network to this address. 
                Sending USDC from other networks (like Ethereum) may result in permanent loss of funds.
              </Text>
            </View>
          </View>
