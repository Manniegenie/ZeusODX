// app/deposits/USDT-arbitrum.tsx
...
export default function UsdtArbitrumDepositScreen() {
  ...
  useEffect(() => {
    const fetchArbitrumAddress = async (): Promise<void> => {
      const cachedAddress = getCachedAddress('USDT', 'ARBITRUM');
      if (cachedAddress) {
  ...
  const handleGetArbitrumAddress = async (forceRefresh = false): Promise<void> => {
    try {
      const result = await getDepositAddress('USDT', 'ARBITRUM', forceRefresh);
      if (result.success) {
        setDepositData(result.data);
        setShowError(false);
      } else {
        showErrorMessage(result.error || 'Failed to get Arbitrum USDT deposit address');
      }
    } catch (error) {
  ...
  const copyToClipboard = async (): Promise<void> => {
    if (!depositData?.address) {
      showErrorMessage('Arbitrum USDT wallet address is not yet available');
      return;
    }
    try {
  ...
  const isLoading = isAddressLoading('USDT', 'ARBITRUM') || supportedLoading;
  const addressError = getAddressError('USDT', 'ARBITRUM');
