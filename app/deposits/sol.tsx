...
export default function SolDepositScreen() {
  ...
  useEffect(() => {
    const fetchSOLAddress = async (): Promise<void> => {
      const cachedAddress = getCachedAddress('SOL', 'SOL');
      if (cachedAddress) {
  ...
  const handleGetSolanaAddress = async (): Promise<void> => {
    try {
      ...
      if (result.success) {
        ...
        showErrorMessage(result.error || 'Failed to get SOL deposit address');
      }
    } catch (error) {
  ...
  const copyToClipboard = async (): Promise<void> => {
    if (!depositData?.address) {
      showErrorMessage('SOL wallet address is not yet available');
      return;
    }
    try {
  ...
  const isLoading = isAddressLoading('SOL', 'SOL') || supportedLoading;
  const addressError = getAddressError('SOL', 'SOL');
          ...
          <View style={styles.headerSection}>
            <View style={styles.headerContainer}>
              ...
              <View style={styles.headerGroup}>
                <Text style={styles.headerTitle}>Deposit SOL</Text>
                <Text style={styles.headerSubtitle}>Solana</Text>
              </View>

              <View style={styles.headerRight}>
                <HintBulb
                  title="Deposit tip"
                  hint="Only send Solana (SOL) to this address on the Solana network. Sending any other asset or using a different network may result in permanent loss."
                />
              </View>
            </View>
          </View>
            ...
            <View style={styles.qrContainer}>
              {isLoading ? (
                <View style={styles.loadingContainer}>
                  ...
                  <Text style={styles.loadingText}>Loading SOL address...</Text>
                </View>
              ) : qrCodeData ? (
