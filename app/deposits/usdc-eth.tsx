...
export default function UsdcEthDepositScreen() {
  ...
  useEffect(() => {
    const fetchUSDCAddress = async (): Promise<void> => {
      const cachedAddress = getCachedAddress('USDC', 'ETH');
      if (cachedAddress) {
  ...
  const copyToClipboard = async (): Promise<void> => {
    ...
    try {
      ...
      Alert.alert('Copied!', 'USDC (Ethereum) wallet address copied to clipboard');
    } catch (error) {
      showErrorMessage('Failed to copy address to clipboard');
    }
  };

  const isLoading = isAddressLoading('USDC', 'ETH') || supportedLoading;
  const addressError = getAddressError('USDC', 'ETH');
          ...
          <View style={styles.warningSection}>
            <View style={styles.warningContainer}>
              ...
              <Text style={styles.warningText}>
                Only send USDC on the Ethereum network to this address. 
                Sending USDC from other networks (like BSC, Polygon, or Arbitrum) may result in permanent loss of funds.
              </Text>
            </View>
          </View>
