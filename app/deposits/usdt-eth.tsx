...
export default function UsdtEthDepositScreen() {
  ...
  useEffect(() => {
    const fetchUSDTAddress = async (): Promise<void> => {
      const cachedAddress = getCachedAddress('USDT', 'ETH');
      if (cachedAddress) {
  ...
  const handleGetUSDTAddress = async (): Promise<void> => {
    try {
      const result = await getUSDTAddress('ETH');
      if (result.success) {
  ...
  const copyToClipboard = async (): Promise<void> => {
    ...
    try {
      ...
      Alert.alert('Copied!', 'USDT (Ethereum) wallet address copied to clipboard');
    } catch (error) {
      showErrorMessage('Failed to copy address to clipboard');
    }
  };

  const isLoading = isAddressLoading('USDT', 'ETH') || supportedLoading;
  const addressError = getAddressError('USDT', 'ETH');
          ...
          <View style={styles.warningSection}>
            <View style={styles.warningContainer}>
              ...
              <Text style={styles.warningText}>
                Only send USDT on the Ethereum network to this address. 
                Sending USDT from other networks (like BSC, Tron, or Polygon) may result in permanent loss of funds.
              </Text>
            </View>
          </View>
