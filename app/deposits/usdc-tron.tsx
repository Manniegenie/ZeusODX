...
export default function UsdcTrxDepositScreen() {
  ...
  useEffect(() => {
    const fetchTrxAddress = async (): Promise<void> => {
      const cachedAddress = getCachedAddress('USDC', 'TRX');
      if (cachedAddress) {
  ...
  const handleGetTrxAddress = async (): Promise<void> => {
    try {
      const result = await getDepositAddress('USDC', 'TRX');
      if (result.success) {
        setDepositData(result.data);
        setShowError(false);
      } else {
        showErrorMessage(result.error || 'Failed to get Tron USDC deposit address');
      }
    } catch (error) {
  ...
  const copyToClipboard = async (): Promise<void> => {
    if (!depositData?.address) {
      showErrorMessage('Tron USDC wallet address is not yet available');
      return;
    }
    try {
  ...
  const isLoading = isAddressLoading('USDC', 'TRX') || supportedLoading;
  const addressError = getAddressError('USDC', 'TRX');
        ...
        >
          <View style={styles.headerSection}>
            ...
            <View style={styles.headerGroup}>
              <Text style={styles.headerTitle}>Deposit USDC (TRX)</Text>
            </View>
            <View style={styles.headerSpacer} />
          </View>

          <View style={styles.subtitleSection}>
            <Text style={styles.subtitle}>Scan the QR code to get Tron (TRC20) deposit address</Text>
          </View>
            ...
            <View style={styles.qrContainer}>
              {isLoading ? (
                <View style={styles.loadingContainer}>
                  ...
                  <Text style={styles.loadingText}>Loading Tron address...</Text>
                </View>
              ) : qrCodeData ? (
          ...
          <View style={styles.warningSection}>
            ...
            <Text style={styles.warningText}>
              • Only send USDC on Tron (TRC20) network to this address{'\n'}
              • Sending from other networks may result in loss of funds{'\n'}
              • Ensure your wallet supports TRC20 before sending
            </Text>
          </View>
          ...
          <View style={styles.shareSection}>
            <TouchableOpacity
              ...
              onPress={async () => {
                if (!depositData?.address) {
                  showErrorMessage('USDC wallet address is not yet available');
                  return;
                }
