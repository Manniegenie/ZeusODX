// app/user/USDCWalletScreen.tsx - Updated to match SolanaWalletScreen styling
...
const USDCWalletScreen = ({ onQuickActionPress, onSeeMorePress }) => {
  ...
  const {
    ...
    hasTransactions,
    refreshTransactions
  } = useHistory('USDC', { defaultPageSize: 5 });

  useEffect(() => {
    if (openNetworkModal === 'true') setShowNetworkModal(true);
  }, [openNetworkModal]);
  ...
  const handleQuickAction = (actionId) => {
    if (actionId === 'deposit') {
      ...
      router.push({ pathname: '../user/Swap', params: { defaultToken: 'USDC' } });
    } else {
  ...
  const handleTransferMethodSelect = (method) => {
    const token = { id: 'usdc', name: 'USD Coin', symbol: 'USDC' };
  ...
  const displayUsdcBalance = formattedUsdcBalance || '0.00';
  const displayUsdBalance = formattedUsdcBalanceUSD || '$0.00';
          ...
          <View style={styles.headerSection}>
            <View style={styles.headerContainer}>
              ...
              <View style={styles.headerGroup}>
                <Image source={usdcIcon} style={styles.iconImage} />
                ...
                <View style={styles.balanceContent}>
                  {loading && !usdcBalance ? (
                    ...
                    <>
                      ...
                      <Text style={styles.balanceAmount}>USDC {displayUsdcBalance}</Text>
