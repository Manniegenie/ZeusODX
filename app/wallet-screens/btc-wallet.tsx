...
// -------------------- Helpers (aligned with History/Avax/BNB) --------------------
...
const BitcoinWalletScreen = ({ onQuickActionPress, onSeeMorePress }) => {
  ...
  const {
    ...
    hasTransactions,
    refreshTransactions
  } = useHistory('BTC', { defaultPageSize: 5 });
  ...
  const handleQuickAction = (actionId: string) => {
    if (actionId === 'deposit') {
      ...
      router.push({ pathname: '../user/Swap', params: { defaultToken: 'BTC' } });
    } else {
  ...
  const handleTransferMethodSelect = (method: TransferMethod) => {
    const token = { id: 'btc', name: 'Bitcoin', symbol: 'BTC' };
    ...
    <View style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        ...
        >
          ...
          {/* Recent History -> tap to open TransactionReceipt with params */}
