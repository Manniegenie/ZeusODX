// app/user/MaticWalletScreen.tsx - Updated to match SolanaWalletScreen styling
...
const MaticWalletScreen = ({ onQuickActionPress, onSeeMorePress }) => {
  ...
  const {
    ...
    hasTransactions,
    refreshTransactions
  } = useHistory('MATIC', { defaultPageSize: 5 });
  ...
  const handleQuickAction = (actionId) => {
    if (actionId === 'deposit') {
      ...
      router.push({ pathname: '../user/Swap', params: { defaultToken: 'MATIC' } });
    } else {
  ...
  const handleTransferMethodSelect = (method: TransferMethod) => {
    const token = { id: 'matic', name: 'Polygon', symbol: 'MATIC' };
