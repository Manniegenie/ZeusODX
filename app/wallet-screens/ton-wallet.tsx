...
const displayBillType = (tx: any): string | undefined => {
  const d = tx?.details || {};
  const raw =
    tx?.billType || tx?.utilityType || d.billCategory || d.billType ||
    d.productName || tx?.category || '';
...
const TONWalletScreen = ({ onQuickActionPress, onSeeMorePress }) => {
  ...
  const handleQuickAction = (actionId: string) => {
    if (actionId === 'deposit') {
      ...
      router.push({ pathname: '../user/Swap', params: { defaultToken: 'TON' } });
    } else {
  ...
  const handleTransferMethodSelect = (method: TransferMethod) => {
    const token = { id: 'ton', name: 'Toncoin', symbol: 'TON' };
    if (method.id === 'zeus') {
      router.push({
  ...
  const handleNetworkSelect = (network: any) => {
    if (network.id === 'ton') router.push('/deposits/ton');
  ...
  const handleViewAllTransactions = () => {
    router.push({
      ...
      params: { currency: 'TON', tokenName: 'Toncoin', tokenSymbol: 'TON' }
    });
  };

  const displayTonBalance = formattedTonBalance || '0.0000';
  const displayUsdBalance = formattedTonBalanceUSD || '$0.00';
          ...
          <View style={styles.headerSection}>
            <View style={styles.headerContainer}>
              ...
              <View style={styles.headerGroup}>
                <Image source={tonIcon} style={styles.iconImage} />
                <Text style={styles.headerTitle}>Toncoin</Text>
              </View>
                ...
                <View style={styles.balanceContent}>
                  {loading && !tonBalance ? (
                    ...
                    <>
                      ...
                      <Text style={styles.balanceAmount}>TON {displayTonBalance}</Text>
...
export default TONWalletScreen;
