...
import {
  ...
  Vibration,
  Keyboard,                 // <--- IMPORTED
  TouchableWithoutFeedback, // <--- IMPORTED
} from 'react-native';
...
export default function VerifyEmailOtpScreen() {
  ...
  const handleOtpChange = (index: number, value: string) => {
    ...
    // Dismiss keyboard only if the last digit is filled
    if (clean && index === OTP_LENGTH - 1) {
      Keyboard.dismiss();
    }
  };
  ...
  const handleVerify = async () => {
    Keyboard.dismiss(); // Dismisses keyboard when Verify is clicked
    hideError();
    const code = otp.join('');
...
const makeStyles = (colors: AppColors) => StyleSheet.create({
  ...
  filledInput: { borderColor: colors.primary, backgroundColor: colors.iconBg },
