...
export default function PassportVerify() {
  ...
  const handlePassportChange = (value: string) => {
    const formatted = formatIdNumber('passport', value);
  ...
  const validation = useMemo(() => {
    ...
    return validatePassport ? validatePassport(passportNumber) : { valid: false, message: '' };
  ...
  const handleBiometricSubmit = async () => {
    ...
    try {
      console.log('🔐 Submitting Passport verification...');
  ...
  const handlePassportSubmit = async () => {
    if (!isValidFormat) {
      openError(validation?.message || 'Passport must be in format: A12345678 or B12345678');
      return;
    }
    ...
    >
      ...
      <View style={styles.section}>
        <Text style={styles.sub}>
          Enter your Nigerian passport number for identity verification.
        </Text>
        ...
        <View style={styles.inputContainer}>
          <TextInput
            ...
            onSubmitEditing={handlePassportSubmit}
          />
          {!isValidFormat && passportNumber.length > 0 && (
            <Text style={styles.errorText}>
              {validation?.message || 'Passport must be in format: A12345678 or B12345678'}
            </Text>
          )}
