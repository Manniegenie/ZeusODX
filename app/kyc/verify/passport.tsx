...
/**
 * UI CONSTANTS
 * Positioning the circle at 25% height ensures the lens aligns 
 * with the face, preventing lower-body captures.
 */
...
export default function PassportVerify() {
  ...
  const handlePassportChange = (value: string) => setPassportNumber(formatIdNumber('passport', value));
  const validation = useMemo(() => (!passportNumber ? { valid: false, message: '' } : validatePassport(passportNumber)), [passportNumber, validatePassport]);
  ...
  const handleBiometricSubmit = async () => {
    setStep('processing');
    try {
      const res = await submitBiometricVerification({ idType: 'passport', idNumber: passportNumber.toUpperCase(), selfieImage: capturedImage! });
      if (res?.success) {
        setShowSuccess(true);
        AppsFlyerService.logEvent('KYC_2', { document_type: 'passport' }).catch(() => {});
      ...
      <View style={styles.section}>
        ...
        <View style={styles.inputContainer}>
          ...
          <TextInput
            ...
            style={[styles.input, !isValidFormat && passportNumber.length > 0 && styles.inputError]}
          />
          {!isValidFormat && passportNumber.length > 0 && <Text style={styles.errorText}>{validation?.message}</Text>}
          {passportNumber.length > 0 && isValidFormat && <Text style={styles.successHint}>✓ Valid passport format</Text>}
        </View>

        <View style={styles.infoBox}>
          <Text style={styles.infoTitle}>💡 Tips for better verification:</Text>
          <Text style={styles.infoText}>• Ensure your passport number is correct and active</Text>
        ...
        <View style={[styles.instructionsContainer, { top: PREVIEW_RECT.minY + PREVIEW_SIZE + 40 }]}>
          {isCountingDown ? <Text style={styles.countdownText}>{countdown}</Text> : (
            <>
              ...
              <Text style={styles.subInstructionsText}>Avoid tilting phone downwards</Text>
            </>
          )}
        </View>
