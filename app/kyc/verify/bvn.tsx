...
export default function BVNVerify() {
  ...
  const { isVerifying, isValidating, submitBiometricVerification, validateBVN, formatIdNumber } = useBiometricVerification();
      ...
      <View style={styles.section}>
        ...
        <View style={styles.inputContainer}>
          ...
          <TextInput
            ...
            style={[styles.input, !isValidFormat && bvn.length > 0 && styles.inputError]}
          />
          {!isValidFormat && bvn.length > 0 && <Text style={styles.errorText}>{validation?.message}</Text>}
          {bvn.length > 0 && isValidFormat && <Text style={styles.successHint}>✓ Valid BVN format</Text>}
        </View>
        ...
        <View style={[styles.instructionsContainer, { top: PREVIEW_RECT.minY + PREVIEW_SIZE + 40 }]}>
          {isCountingDown ? <Text style={styles.countdownText}>{countdown}</Text> : (
            <>
              ...
              <Text style={styles.subInstructionsText}>Avoid tilting the phone downwards</Text>
            </>
          )}
        </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <SuccessModal visible={showSuccess} onClose={() => router.replace('/kyc/kyc-upgrade')} title="BVN Verification in Progress" message="You will get an email with the confirmation status soon." />
    </SafeAreaView>
  );
}
